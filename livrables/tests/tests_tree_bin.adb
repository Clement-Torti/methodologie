...
package body tests_tree_bin is
    procedure run_tests_tree_bin is
        CPT: Integer := 0;
    ...
    begin
        ...
        New_Line;
        Put_Line("-> stringify_tree: success");

        -- equal tree
        b := equal_tree(t, t2);
        assert(not b, "Fail -> error equal_tree -> should be non equals");
        b := equal_tree(t, t);
        assert(b, "Fail -> error equal_tree -> should be equal");

        New_Line;
        Put_Line("-> equal_tree: success");
